...
class Mosquitto():
    def __init__(self, IP, port, topic_, thingsboard, accesstoken=""):
        try:
            ...
            self.hostname = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(7))
            self.IP = IP
            self.port = port
            self.topic = topic_
            self.poll_time = 5
            self.client = mqtt.Client("", userdata=self, protocol=mqtt.MQTTv31)
            #self.client = mqtt.Client("PClient_%s" % self.hostname, userdata=self, protocol=mqtt.MQTTv31)
    ...
    def publish(self, count, inUse, buckets):
        if self.connection is False:
            return
        print('count = %d, inUse = %d' % (count, inUse))
        avg_, max_ = self.avgmax(count)
        d = {"hostname": self.hostname, "peopleCount" : count, "cameraInUse" : inUse, "average": avg_, "maximum": max_, 'zones': buckets}
